...
class AgentBuilderTab(QWidget):
    ...
    def _build_ui(self):
        ...
        _save_icon = os.path.join(_icons_dir, "diskette.png")
        if os.path.isfile(_save_icon):
            save_btn.setIcon(_QIcon(_save_icon))
            save_btn.setIconSize(_QSize(20, 20))
        save_btn.clicked.connect(self._save_agent)
        action_layout.addWidget(save_btn)

        save_run_btn = QPushButton("  Save & Run")
        _run_icon = os.path.join(_icons_dir, "rocket.png")
        if os.path.isfile(_run_icon):
            save_run_btn.setIcon(_QIcon(_run_icon))
            save_run_btn.setIconSize(_QSize(20, 20))
